...
class TestUninstallAll:
    """Tests for the uninstall_all workflow."""

    async def test_disables_extensions_before_loader(self, manager, mock_sys, plugin_dir):
        """Should disable other extensions before the loader."""
        # Setup loader
        mock_sys.add_manifest("loader", {
            "id": "loader",
            "name": "Extension Loader",
        ...
        # Set both as active
        mock_sys.set_active_extensions([
            "steamos-extension-loader",
            "steamos-extension-tailscale"
        ])

        await manager.uninstall_all()

        # Verify tailscale was removed before loader
        remove_order = mock_sys.get_remove_order()
        assert "tailscale" in remove_order
        assert "loader" in remove_order
        assert remove_order.index("tailscale") < remove_order.index("loader")

    async def test_reboots_after_uninstall(self, manager, mock_sys, plugin_dir):
        """Should trigger reboot after uninstalling all extensions."""
        mock_sys.add_manifest("loader", {
            "id": "loader",
            "name": "Extension Loader",
            "activation": {"mode": "reboot"}
        ...
        assert mock_sys.reboot_called

    async def test_skips_disabled_extensions(self, manager, mock_sys, plugin_dir):
        """Should not try to disable already-disabled extensions."""
        # Loader is installed and active
        mock_sys.add_manifest("loader", {
            "id": "loader",
            "name": "Extension Loader",
            "activation": {"mode": "reboot"}
        ...
        # Tailscale manifest exists but not installed (disabled)
        mock_sys.add_manifest("tailscale", {
            "id": "tailscale",
            "name": "Tailscale VPN",
            "activation": {"mode": "auto"}
        ...
        mock_sys.add_bundled_raw("tailscale", plugin_dir)
        # Note: NOT adding installed raw for tailscale

        mock_sys.set_active_extensions(["steamos-extension-loader"])

        await manager.uninstall_all()

        # Only loader should have been removed
        remove_order = mock_sys.get_remove_order()
        assert "loader" in remove_order
        assert "tailscale" not in remove_order

    async def test_disables_sysext_when_all_removed(self, manager, mock_sys, plugin_dir):
        """Should disable systemd-sysext after removing all extensions."""
        mock_sys.add_manifest("loader", {
            "id": "loader",
            "name": "Extension Loader",
            "activation": {"mode": "reboot"}
